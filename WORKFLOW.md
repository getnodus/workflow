# Intuitumxyz GitHub workflow control plane

This repository is the shared GitHub *automation* home for the `Intuitumxyz` org.
Org *identity* (profile, issue templates, community-health files) lives in
`Intuitumxyz/.github`, which GitHub renders specially. Two ideas keep this place
small:
...
`github>Intuitumxyz/workflow`) and **`REPO_STANDARD.md`** (the default shape for a
Intuitumxyz repo).
...
- **Self-hosted** (`runs-on: self-hosted`) is for heavy, trusted builds that are
  slow or costly on hosted runners — today, `Intuitumxyz/solo` (the VS Code fork).
...
3. `renovate.json`: `{ "extends": ["github>Intuitumxyz/workflow"] }`.
